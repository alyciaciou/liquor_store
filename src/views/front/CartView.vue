<template>
    <div class="bg-[#070707f0] text-white">
        ...
        <header>
            <div class="h-[500px] bg-cover bg-center flex flex-col items-center justify-center text-center opacity-85" style="background-image: url(https://storage.googleapis.com/vue-course-api.appspot.com/liquor_store/1709970265168.jpg?GoogleAccessId=firebase-adminsdk-zzty7%40vue-course-api.iam.gserviceaccount.com&Expires=1742169600&Signature=GgDxE7tAAcik7tc5hdjwMo%2FyjppYonHnslXCanpx5IbSEb0bJtENREUymOqsh%2F%2B4DJ0JNI79J6tBBrpUcX6lX7a%2BJPL4DHffzSpAYlyYM5LCjW0B4CptkDUYzIs8lGsFROiN8ihnKQKEpUdBMpKG9KDz0cFc8nYIoXYCjXC6oy65nmQJvVbzk7q1H%2BiYkvT5Pz1b1f%2BAAFTxjZyTK%2Bs3OvlCeCu0MGG%2FPsL4feqCphFy%2FDBU3b8p98UI3UZXfqhopN3cZ85MX82EZKs4Z85fW83iLRixpdrGDRzOqOORd9O57mKTAGVCB3JAf1ntnksKrzWWsqWsDLZWcd8DI1vymg%3D%3D);">
                <div class="">
        ...
        <main class="container mx-auto p-6 md:p-24 mb-6">
            <table v-if="info !== null && info.carts.length !== 0" class="border-t-2 w-full py-8">
                    ...
                    <tr class="border-b-2" v-for="(item, index) in info.carts" :key="item">
                        <td class="w-[24%] p-2">
                            <div class="flex flex-col items-center justify-between">
                                <img :src="item.product.imageUrl" class="rounded-lg md:h-[200px] md:w-[160px] object-cover mb-2" :alt="item.product.title">
                        ...
                        <td class="p-2 w-[24%]">
                            <div class="border-2 rounded-lg mb-4 md:mb-0 flex items-center">
                                <button :disabled="item.qty === 1 || isDisabled" @click="minusNum(item, index)" class="duration-500 hover:bg-white hover:text-black font-extrabold text-white py-2 rounded-l-lg w-[20%] text-center min-w-[30px]" type="button">-</button>
                                <img v-if="isChangeNum && itemIndex === index" class="h-6 w-6 mx-auto " src="/Rolling-1s-150px.gif" alt="loading">
                                <input @change="changeNum($event, item, index)" v-else :value="item.qty" type="text" class=" text-center bg-gray-800  p-2 w-[60%] min-w-[30px]">
                                <button :disabled="isDisabled" @click="addNum(item, index)" class="duration-500 hover:bg-white hover:text-black font-extrabold text-white py-2 rounded-r-lg w-[20%] text-center min-w-[30px]" type="button">+</button>
                            </div>
                        </td>
                ...
                <tfoot class="text-right">
                    <tr>
                        <td colspan="6" class="p-6 text-3xl"><strong>總計：</strong> NT${{ info. final_total}}</td>
                    </tr>
                </tfoot>
            </table>
